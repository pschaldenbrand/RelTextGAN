...
# https://github.com/elvisyjlin/RelGAN-PyTorch/blob/master/nn.py
...
class Discriminator(nn.Module):
    def __init__(self, img_size, nc=4, ndf=64):
        super(Discriminator, self).__init__()
...
class ImageToEncoding(nn.Module):
    def __init__(self, img_size=128, encoding_size=768, nc=3, ndf=64):
        super(ImageToEncoding, self).__init__()
        self.img_size = img_size
        if img_size == 128:
            self.main = nn.Sequential(
                # input is (nc) x 128 x 128
                ...
                # nn.Sigmoid()
                nn.Flatten(),
                nn.Linear(ndf * 8 * 4 * 4, encoding_size)
            )
        elif img_size == 256:
            self.main = nn.Sequential(
                # input is (nc) x 256 x 256
