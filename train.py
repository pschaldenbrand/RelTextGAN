...
parser.add_argument('--image_size', type=int, default=128, help='Image Dimensions. Used for both height and width, sorry.')
...
writer = SummaryWriter(log_dir='logdir/{}'.format(run_name))
global_step = 0
...
for epoch in range(666):
    ...
    for i in tqdm(range(0, int(len(train_ids)/batch_size)), desc="Epoch {}".format(epoch)):
        ...
        #l_g_total = l_cycle + l_g_align + l_g_real + .1*l_g_img2enc#l_cycle + 0.5*l_g_align + l_g_real + .1*l_g_img2enc
